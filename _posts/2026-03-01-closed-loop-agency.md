---
layout: post
title: "閉ループの同型性と閉包性──AIエージェントと人間の本当の差分"
date: 2026-03-01
...
知覚入力を「世界の状態」として内部表現にするプロセスと、その見立てに価値判断を与えるプロセスの二段だ。この二段を分離して描く利点は後述する。なお、状態更新 $$u$$ の形が閉ループのステートフル性を決める。

- **反射的（ステートレスに近い）**：完全観測で単発入力が足りるなら $$s_t = o_t$$
- **ステートフル**：部分観測なら $$s_t = u(s_{t-1}, o_t)$$。$$s_t$$ が過去の十分統計量（要約）になる。
...
$$\delta$$ は枠組みによって異なる名前を持つ──RLなら報酬予測誤差、制御理論なら制御誤差、FEP（自由エネルギー原理）なら予測誤差、viability理論なら制約違反フラグ──が、「期待と現実のずれ」という構造は共通だ。

### Encode / Act / Adapt を分離して描く利点

表現（$$u$$: 状態推定）と評価（$$f$$: 価値づけ）を分けておく利点は、閉包性の議論に直結する。AIエージェント系はしばしば「$$u$$ は内部で回るが、$$f$$ や更新 $$\Delta$$ が外部に散る」という分離が起きる。人間は $$u, f, \Delta$$ が身体内で高密度に結びつきやすい──この差分が、分離した表記で初めて明示的に描ける。
...
### マッピング：Codex / Claude Code / 生物脳

同じ数式の各変数が、具体的な実体にどう対応するかを並べる。

<div style="overflow-x: auto;">
<table>
<thead>
<tr><th>変数</th><th>OpenAI Codex</th><th>Claude Code</th><th>生物脳</th></tr>
</thead>
<tbody>
<tr>
<td markdown="span">$$o_t$$（観測）</td>
<td>ユーザー指示＋リポジトリ状態＋コマンド実行結果＋テスト結果。ローカルの選択ディレクトリ内で「読める・書ける・実行できる」前提 <sup><a href="#ref-19">(19)</a></sup></td>
<td>ユーザー指示＋コードベース＋コマンド結果。「読み、編集し、コマンドを走らせる」ツール <sup><a href="#ref-22">(22)</a></sup></td>
<td>感覚入力（視覚・聴覚・内受容感覚など）</td>
</tr>
<tr>
<td markdown="span">$$s_t = u(s_{t-1}, o_t)$$（状態）</td>
<td>会話履歴＋ファイル差分＋ツール結果＋要約。AGENTS.md が初期条件（事前コンテキスト）として効く <sup><a href="#ref-14">(14)</a></sup>。sandbox と approval policy が観測・行為の制約 <sup><a href="#ref-21">(21)</a></sup></td>
<td>会話＋コード＋実行結果に加え、メモリ用ディレクトリの読み書き＋MEMORY.md。auto memory は machine-local で共有範囲が限定される <sup><a href="#ref-16">(16)</a></sup></td>
<td>強いステートフル性（反回路＋記憶系）。海馬リプレイが過去を圧縮して運ぶ <sup><a href="#ref-23">(23)</a></sup></td>
</tr>
<tr>
<td markdown="span">$$v_t = f(s_t)$$（価値）</td>
<td>「テストが通る・要求を満たす・安全制約に違反しない・スタイル規約」。多くは AGENTS.md／指示文／approval policy／テストハーネスとして外部に実装 <sup><a href="#ref-14">(14)</a></sup></td>
<td>ユーザー要件＋プロジェクト規約＋安全制約＋成功条件。価値の多くが外部（規約・ツールチェーン・レビュー）に置かれる</td>
<td>接近／回避、快・不快、欲求、危険度など評価機構の合成。報酬予測誤差が学習の中核シグナル <sup><a href="#ref-24">(24)</a></sup></td>
</tr>
<tr>
<td markdown="span">$$a_t$$（行動）</td>
<td>ファイル編集、コマンド実行、PR作成。クラウド版はsandbox内でタスク実行 <sup><a href="#ref-20">(20)</a></sup></td>
<td>ファイル編集、コマンド実行、Git操作</td>
<td>身体運動、発話、道具操作</td>
</tr>
<tr>
<td markdown="span">$$\delta$$（誤差）</td>
<td>テスト失敗、ビルド失敗、lint、ユーザーの否定フィードバック、approval拒否</td>
<td>テスト失敗、コマンドエラー、ユーザー修正指示</td>
<td>報酬予測誤差（ドーパミン）、制御誤差、痛み信号</td>
</tr>
<tr>
<td markdown="span">$$\theta$$ と $$\Delta$$（更新）</td>
<td>セッション内ではLLM重み \(\theta\) は固定。更新は主に外部状態へ：修正コミット、メモ書き、AGENTS.md追記。MCP化で外部状態の持続を強化可能 <sup><a href="#ref-25">(25)</a></sup></td>
<td>セッション内の主更新先はメモリファイルやコード側。重み更新は通常セッション外（モデル更新）</td>
<td>誤差がシナプス可塑性＝パラメータ更新に直結。LTP/LTD が長期的な結合強度変化 <sup><a href="#ref-3">(3)</a></sup></td>
</tr>
</tbody>
</table>
</div>
...
- **Codex / Claude Code**：Adaptは主に外部状態（コード、メモリ、規約ファイル、テストハーネス、approval）へ流れ、$$\theta$$ 更新は境界外（モデル運営側）にあることが多い。
